...
<li x-data="{ label: @js($subNavigation ? "sub_navigation_{$label}" : $label) }" data-group-label="{{ $subNavigation ? "sub_navigation_{$label}" : $label }}"
	{{ $attributes->class(['fi-sidebar-group flex flex-col gap-y-1', 'fi-active' => $active]) }}>
	@if ($label)
		<div @if ($collapsible) x-on:click="$store.sidebar.toggleCollapsedGroup(label)" @endif
			...
			@if ($icon)
				<x-filament::icon :icon="$icon" class="fi-sidebar-group-icon h-6 w-6 text-gray-400 dark:text-gray-500" />
			@endif
			...
			@if ($collapsible)
				<x-filament::icon-button color="blue" icon="heroicon-m-chevron-up" icon-alias="panels::sidebar.group.collapse-button"
					:label="$label" x-bind:aria-expanded="!$store.sidebar.groupIsCollapsed(label)"
					x-on:click.stop="$store.sidebar.toggleCollapsedGroup(label)" class="fi-sidebar-group-collapse-button"
	...
	@if ($hasDropdown)
		<x-filament::dropdown :placement="(__('filament-panels::layout.direction') === 'rtl') ? 'left-start' : 'right-start'" teleport x-show="! $store.sidebar.isOpen">
			<x-slot name="trigger">
			...
			@foreach ($lists as $list)
				<x-filament::dropdown.list>
					@foreach ($list as $item)
						...
						<x-filament::dropdown.list.item :badge="$item->getBadge()" :badge-color="$item->getBadgeColor()" :badge-tooltip="$item->getBadgeTooltip()" :color="$itemIsActive ? 'primary' : 'gray'"
							:href="$item->getUrl()" :icon="$itemIsActive ? $item->getActiveIcon() ?? $item->getIcon() : $item->getIcon()" tag="a" :target="$item->shouldOpenUrlInNewTab() ? '_blank' : null">
							{{ $item->getLabel() }}
	...
	@endif
	<ul
		@if (filled($label)) @if ($sidebarCollapsible) x-show="$store.sidebar.isOpen ? ! $store.sidebar.groupIsCollapsed(label) || @js($active) : !@js($hasDropdown)" @else x-show="! $store.sidebar.groupIsCollapsed(label) || @js($active)" @endif
		x-collapse.duration.200ms @endif
		@if ($sidebarCollapsible) x-transition:enter="delay-100 lg:transition"
            x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" @endif
